 ...
 * 모델: gemini-2.5-flash (Stable, Structured outputs 지원)
 * 
 * ⚠️ 중요: generateObject는 반드시 Zod schema를 사용해야 합니다.
 * JSON Schema 객체를 전달하면 typeName undefined 에러가 발생합니다.
 * 
 * 참고: gemini-1.5-flash는 더 이상 지원되지 않아 gemini-2.5-flash로 변경되었습니다.
 */
...
export async function callGoogleAI(
  ...
  // @ai-sdk/google는 process.env.GOOGLE_GENERATIVE_AI_API_KEY를 자동으로 읽음
